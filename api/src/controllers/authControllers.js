...
const userRegist = async (req, res) => {
  ...
  console.log("userRegister controllers", req.body.mail);

  const prevUser = await User.findOne({ where: { mail: mail } }); console.log(prevUser);
  ...
  res.send({
    msj: 'verifique su mail para confirmar el registro',
    username: mail,
    name: userCreate.name,
    token
  });
